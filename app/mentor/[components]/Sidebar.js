                ...
                <div className={`${isSidebarOpen ? 'block' : 'hidden'} lg:block`}>
                    ...
                    <div className='min-h-[90vh] rounded-[8px] relative' style={{ background: 'linear-gradient(176.83deg, #52E7CF 0.29%, #0096F6 3.41%, #1DB3E8 57.31%, #52E7CF 85.81%)' }}>
                            ...
                            <ul className='flex flex-col gap-2'>
                                {[
                                    { name: 'Dashboard', icon: <Image src={dashboardImg} alt="Dashboard Icon" width={20} height={20} />, path: '/mentor/dashbord' },
                                    { name: 'Session', icon: <Image src={openBook} alt="Session Icon" width={20} height={20} />, path: '/mentor/session' },
                                    { name: 'Session Calendar', icon: <FaRegCalendarDays className="w-5 h-5" />, path: '/mentor/calender' },
                                    { name: 'Notice Board', icon: <IoMdClipboard className="w-5 h-5" />, path: '/mentor/noticeboard' },
                                    { name: 'Messages', icon: <AiOutlineMessage className="w-5 h-5" />, path: '/mentor/chat' },
                                    { name: 'My Account', icon: <IoMdContact className="w-5 h-5" />, path: '/mentor/account' },
                                    { name: 'My Withdrawal', icon: <Image src={walletImg} alt="My Withdrawal Icon" width={20} height={20} />, path: '/mentor/withdrawls' }
                                ].map((item, index) => (
