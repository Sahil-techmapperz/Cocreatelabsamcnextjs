...
export async function PATCH(req,{params}) {
  ...
  try {
    ...
    if (!Client || Client.role !== 'Client') {
      return NextResponse.json({ message: 'Only Mentee are allowed to cancel sessions' }, { status: 403 });
    }
    ...
    const Newsessions = await Sessions.find({ mentor: userId }).populate('mentor', 'name profilePictureUrl');
